...
class ColumnDescriptionChecker:
    ...
    def get_meas_type(self):
        return self.kw['MEASINFO']['type'] \
            if self.is_meas() is True else None

    def get_meas_ref(self):
        return self.kw['MEASINFO']['Ref'] \
            if self.is_meas() else None

    def get_meas_unit(self):
        return self.kw['QuantumUnits'] \
            if self.is_meas() else None

    def is_epoch_meas(self):
        return self.is_meas() and \
            (self.get_meas_type() == 'epoch')

    def is_direction_meas(self):
        return self.is_meas() and \
            self.get_meas_type() == 'direction'

    def is_position_meas(self):
        return self.is_meas() and \
            self.get_meas_type() == 'position'

    def is_uvw_meas(self):
        return self.is_meas() and \
            self.get_meas_type() == 'uvw'
