...
def _get_feed_columns(hdu: 'BinTableHDU') -> dict:
    ...
    beam_offset = [
        np.zeros((2, 2), dtype=float) for _ in range(num_beam)
    ]
    ...
    if rx_str[0][-1] in ('H', 'V'):
        ...
        _pol_type = ['R', 'L']
    pol_type = [
        np.array(_pol_type) for _ in range(num_beam)
    ]

    pol_response = [
        np.zeros((2, 2), dtype=complex) for _ in range(num_beam)
    ]

    feed_position = [
        np.zeros(3, dtype=float) for _ in range(num_beam)
    ]

    receptor_angle = [
        np.zeros(2, dtype=float) for _ in range(num_beam)
    ]
